...
var actions = {
  addCart (e){
    ...
    .then(response => {
      datos.status = response.clone().status // 200 a 400
    ...
    .then( response => {
      ...
      if (datos.status == 200) {
        ...
        if (response.update_cart == 0)
          ...
          document.getElementById('mycartTotal').textContent = datos.total - datos.product.p_price
          document.getElementById('mycart').textContent = response.cart;
          ap_accions.toast_alert("success", response.response, 2000)
      }else{
        ap_accions.toast_alert("success", response.products[0], 2000)
      }
    })
    .catch(console.log)
  },
  ...
  deleteFavorite(e,fila){
    ...
    .then( result => {
      if (datos.status == 200) {
        ap_accions.toast_alert("success", result.response, 2000)
        datos.element.style.display = "none"
      }else{
        ap_accions.toast_alert("success", result.products[0], 2000)
      }
    })
    .catch(console.log)
  }
